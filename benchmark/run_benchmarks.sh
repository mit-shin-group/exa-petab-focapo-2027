...
#   the shared warmup model -> PEtab (petab_<optimizer>_*, each optimizer in series) -> ExaGPU
#   (exagpu_*) -> ExaCPU (exacpu_*), then regenerates the report + figure. Resumable: terminal
# results are skipped. results_table.jl / results_plot.jl assemble from that one dir.
#
...
#   GPU_ID=0 bash run_benchmarks.sh           # device for the warmup and the GPU sweep (default 1)
#
...
GPU_ID=${GPU_ID:-1}      # device the warmup and the GPU sweep both run on
...
# ── 1. Warmup model (BENCH_WARMUP_MODEL) — only the side this tag runs ─────────────
echo "[1/5] warmup ..."
if [ "$INC_GPU" = true ] && [ "$GPU" = true ]; then
    retry 100 env OMP_NUM_THREADS="$THREADS" OPENBLAS_NUM_THREADS="$THREADS" \
        julia --project=. -t 1 "$HELP/run_warmup.jl" "$GPU_ID" > "$LOG/warmup_gpu.log" 2>&1
fi
...
# ── 2. PEtab.jl (CPU) ────────────────────────────────────────────────────────────
if [ "$INC_PETAB" = true ]; then
    echo "[2/5] PEtab.jl ($TAG) ..."
    bash "$HELP/run_petab.sh" 1   # PAR=1: serial, uncontended timing
else
    echo "[2/5] PEtab.jl SKIPPED (tag $TAG)"
fi

# ── 3. ExaModels + MadNLP, GPU (CUDSS) ──────────────────────────────────────────
if [ "$INC_GPU" = true ] && [ "$GPU" = true ]; then
    echo "[3/5] ExaModels GPU ..."
    bash "$HELP/run_examodels.sh" 1 "$GPU_ID"   # NINST=1: single GPU instance, uncontended timing
elif [ "$INC_GPU" = true ]; then
    echo "[3/5] ExaModels GPU SKIPPED (no functional GPU)"
else
    echo "[3/5] ExaModels GPU SKIPPED (tag $TAG)"
fi

# ── 4. ExaModels + MadNLP, CPU (HSL) — CPU_INST instances strided over the model set ─
if [ "$INC_CPU" = true ]; then
    echo "[4/5] ExaModels CPU (HSL) x$CPU_INST ..."
    ...
    echo "[4/5] ExaModels CPU SKIPPED (tag $TAG)"
fi
