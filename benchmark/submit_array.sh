...
warmup=$(sbatch --parsable -J exa-warmup -t 6:00:00 run_benchmarks_array.sbatch warmup)
models=$(sbatch --parsable -J exa-models --array=1-${#MODELS[@]}%$PARALLEL --dependency=afterok:$warmup run_benchmarks_array.sbatch "${MODELS[@]}")
sbatch -J exa-report -t 1:00:00 --dependency=afterany:$models run_benchmarks_array.sbatch report
